...
**800×600** canvas, position/scale it, run it through a **dither → color → CRT**
...
1. **rasterize** — draw the placed sprite at its position/scale into the
   800×600 buffer (nearest-neighbor, full color). The source of truth.
2. **levels** — black point / white point / gamma (LUT, pre-dither).
3. **dither** — FS error diffusion or Bayer 2/4/8 ordered. Color ON → snaps to
   the palette; Color OFF → 1-bit with a threshold cutoff. Bayer is keyed to
   absolute canvas x,y, so the pattern stays locked to the grid as the sprite
   moves ("re-dither as it moves").
...
per animation frame, paints the flat 800×600 canvas (CSS-scaled, crisp) when CRT
...
— an array, not a rewrite. `x, y` are the sprite **center** in 800×600 source
space; `scale` is a free multiplier on natural size.

## Export rules

- **CRT OFF** → flat 800×600; background **filled** or **transparent** (toggle).
- **CRT ON** → CRT baked in, background **forced opaque**, export scale
  **1× / 2× / 3×** (default 2× ≈ 1600×1200).
