...
// v2 prompts share a style/framing base; they differ only in exaggeration.
const V2_STYLE =
  "in the exact style of the single attached reference image: flat, " +
  "illustrative BLACK AND WHITE (grayscale only, no colour), bold cel shading " +
  "with subtle painterly texture and a clean white keyline around the figure. " +
  "This is NOT anime or manga — avoid big anime eyes and cartoon proportions; " +
  "keep it a grounded, semi-realistic illustrated caricature exactly like the " +
  "reference.";
const V2_TAIL =
  ...
  "uniform #0047BB blue background — no other colours, no texture, no scenery, " +
...
const V2_SUBTLE =
  `Redraw this person as a caricature ${V2_STYLE} Slightly exaggerate the most ` +
  `distinctive features while keeping the person clearly recognizable. ${V2_TAIL}`;

const V2_BOLD =
  `Redraw this person as a caricature ${V2_STYLE} Push the caricature harder — ` +
  `noticeably exaggerate and distort the most prominent features (nose, jaw, ` +
  `brow, ears, hairline) for a bold, characterful, slightly comic exaggeration, ` +
  `while keeping the person clearly recognizable. ${V2_TAIL}`;
...
export const FACE_VERSIONS: FaceVersion[] = [
  ...
  },
  {
    ...
    label: "V2",
    prompts: [
      { label: "SUBTLE", text: V2_SUBTLE },
      { label: "BOLD", text: V2_BOLD },
    ],
